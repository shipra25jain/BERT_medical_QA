...
"""script to train the BERT based model for machine comprehension task on emrQA dataset (electronic medical records) on TPUs. This has one extra classification layer compared to BERT QA model for SQUAD"""
...
flags.DEFINE_string("predict_precomputed_file", None,
                    "Precomputed tf records for prediction.Do not store any information for supervision")
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def compute_label_loss(logits, labels):
                ...
                loss = tf.contrib.losses.softmax_cross_entropy(logits, one_hot_labels, weights=weights)
                return loss

            start_positions = features["start_positions"]
            end_positions = features["end_positions"]
            has_answer = features["has_answer"]
            start_loss = compute_loss(start_logits, start_positions)
            end_loss = compute_loss(end_logits, end_positions)
            has_answer_loss = compute_label_loss(has_answer_logits,has_answer)
            total_loss = (start_loss+end_loss+has_answer_loss)/3.0
